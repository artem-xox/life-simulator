...
|---|---|---|
| 7.1 | Replace `Biome` enum with `Surface` (`OCEAN`, `FRESH_WATER`, `SAND`, `FOREST`) + lookup tables in `config/settings.py` | old biome constants gone; tests updated |
| 7.2 | Island generator: fBm elevation × radial falloff → land mask, ocean all around | every map edge is `OCEAN`; land is one dominant island |
| 7.3 | Lakes (depressions below lake level) and 2–4 rivers traced downhill to the ocean as `FRESH_WATER` | at least one lake or river on default seed; rivers reach the ocean |
...
| 7.7 | Terrain render v2: grass density shading, water depth tint, shoreline edge, seeded decorative trees | grazing scars visibly change the map during a run |
